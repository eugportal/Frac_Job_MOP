...
export async function getCompanyWells(accessToken: string): Promise<Array<{ value: string; label: string }>> {
  const response = await fetch(`${apiUrl}/api/companies/me/wells`, { headers: { Authorization: `Bearer ${accessToken}` } });
  const payload = await response.json().catch(() => ({})) as { message?: string; wells?: Array<{ name: string }> };
  if (!response.ok) throw new Error(payload.message ?? 'Unable to load company wells.');
  return (payload.wells ?? []).map((well) => ({ value: well.name, label: well.name }));
}
