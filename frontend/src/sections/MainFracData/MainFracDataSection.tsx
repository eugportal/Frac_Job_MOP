    ...
    <div className="flex flex-col gap-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <NumberField placeholder='ex.33.33214555' label="Latitude ex.33.33214555" name="latitude" value={w.latitude} step={0.000001} onChange={(v) => updateWellInfo('latitude', v)} />
          <NumberField placeholder='ex.33.33214555' label="Longitude ex.33.33214555" name="longitude" value={w.longitude} step={0.000001} onChange={(v) => updateWellInfo('longitude', v)} />
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <NumberField placeholder='' label="MidPerf TVD" name="midPerfTVD" value={r.midPerfTVD} min={0} unit="ft" help="True vertical depth at the midpoint of perforations." error={err('midPerfTVD')} onChange={(v) => updateReservoir('midPerfTVD', v)} />
          <NumberField placeholder='' label="No. of Perforations" name="numberOfPerfs" value={r.numberOfPerfs} min={0} step={1} error={err('numberOfPerfs')} onChange={(v) => updateReservoir('numberOfPerfs', v)} />
          <NumberField placeholder='' label="Max Deviation" name="maxDeviation" value={r.maxDeviation} min={0} max={90} step={0.1} unit="°" help="Maximum wellbore deviation in degrees." error={err('maxDeviation')} onChange={(v) => updateReservoir('maxDeviation', v)} />
          <NumberField placeholder='' label="Avg Reservoir Pressure" name="averageReservoirPressure" value={r.averageReservoirPressure} min={0} unit="psi" error={err('averageReservoirPressure')} onChange={(v) => updateReservoir('averageReservoirPressure', v)} />
          <NumberField placeholder='' label="BHST" name="bhst" value={r.bhst} min={0} unit="°F" help="Bottom hole static temperature." error={err('bhst')} onChange={(v) => updateReservoir('bhst', v)} />
          <NumberField placeholder='' label="Avg Porosity" name="averagePorosity" value={r.averagePorosity} min={0} max={100} step={0.1} unit="%" error={err('averagePorosity')} onChange={(v) => updateReservoir('averagePorosity', v)} />
        </div>
      </section>
...
function StageModal({ open, stage, nextStageNumber, onClose, onSave }: StageModalProps) {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <NumberField placeholder='' label="Stage #" name="stage" value={draft.stage} min={1} step={1} required onChange={(v) => update('stage', v)} />
        <NumberField placeholder='' label="Perf Top" name="perfTop" value={draft.perfTop} min={0} unit="ft" required onChange={(v) => update('perfTop', v)} />
        <NumberField placeholder='' label="Perf Bottom" name="perfBottom" value={draft.perfBottom} min={0} unit="ft" required onChange={(v) => update('perfBottom', v)} />
        <NumberField placeholder='' label="Pad %" name="stagePadPercent" value={draft.padPercent} min={0} max={100} step={0.1} unit="%" onChange={(v) => update('padPercent', v)} />
        <NumberField placeholder='' label="Fluid Volume" name="fluidVolume" value={draft.fluidVolume} min={0} unit="bbl" onChange={(v) => update('fluidVolume', v)} />
        <NumberField placeholder='' label="Proppant" name="proppantAmount" value={draft.proppantAmount} min={0} unit="lb" onChange={(v) => update('proppantAmount', v)} />
        <NumberField placeholder='' label="Rate" name="rate" value={draft.rate} min={0} unit="bpm" onChange={(v) => update('rate', v)} />
        <NumberField placeholder='' label="Pressure" name="pressure" value={draft.pressure} min={0} unit="psi" onChange={(v) => update('pressure', v)} />
      </div>
    </Modal>
  );
}
