...
import { localFracDataService } from '@/services/fracDataService';
...
export function FracDataForm() {
  ...
  const confirmSubmit = async () => {
    const result = await localFracDataService.submit(formData);
...
function SectionBadge({ status, label, required }: { status: string; label: string; required?: boolean }) {
  ...
  return (
    <span className="flex items-center gap-1.5">
      <span className={`badge ${colorClass}`}>{statusText}</span>
