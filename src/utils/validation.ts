...
export function validateMainFracData(data: MainFracData): FieldError[] {
  ...
  if (isBlank(r.jobType))
    errors.push({ field: 'jobType', label: 'Job Type', section, message: 'Job Type is required.' });
  ...
  if (r.stageNumber !== null && r.stageNumber < 1)
    errors.push({ field: 'stageNumber', label: 'Stage #', section, message: 'Stage number must be at least 1.' });
...
export function countFilledFields(data: MainFracData): { filled: number; total: number } {
  ...
  const w = data.wellInfo;
  const wFields = [w.well, w.field, w.regionArea, w.jobDate, w.onOffShore, w.fracVendor, w.rigName, w.dataSourceConfidence];
  wFields.forEach((v) => {
    total++;
    if (!isBlank(v)) filled++;
  });
  ...
  const r = data.reservoir;
  const rFields = [r.formationName, r.lithology, r.wellType, r.jobType];
  rFields.forEach((v) => {
    total++;
    if (!isBlank(v)) filled++;
  });
  const rNums = [r.stageNumber, r.padPercent, r.midPerfTVD, r.numberOfPerfs, r.maxDeviation, r.averageReservoirPressure, r.bhst, r.averagePorosity];
  rNums.forEach((v) => {
    total++;
